module.exports = app => {
  ...
  router.get('/news/list', async (req, res) => {
    ...
    cats.unshift({
      ...
      }).populate('categories').limit(5).sort({'createdAt':-1}).lean()
    })
  ...
  router.get('/heroes/:id', async (req, res) => {
    const data = await Hero
      ...
      .populate('categories items1 items2 partners.hero badPartners.hero restrains.hero')
      .lean()
    res.send(data)
  })

  // 图文数据
  router.get('/picarticles/list', async (req, res) => {
    const parent = await Category.findOne({
      name: '图文分类'
    })
    const cats = await Category.aggregate([
    ...
    cats.map(cat => {
      cat.picNewsList.map(news => {
        news.categoryName = cat.name
        return news
      })
      cat.picNewsList.sort((a,b)=> {
        if (a.createdAt > b.createdAt) {
          return -1;
        }else if (a.createdAt == b.createdAt) {
          return 0;
  ...
  // 图文详情
  router.get('/picarticles/:id', async (req, res) => {
    const data = await PicArticle.findById(req.params.id).lean()
    data.related = await PicArticle.find().where({
      categories: { $in: data.categories }
    }).limit(2)
    res.send(data)
  })
