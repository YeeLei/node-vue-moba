        ...
        <el-form label-width="120px"
          ...
          <el-form-item>
            <el-button type="primary"
                       round
                       @click="submitForm('PicArticleForm')"
...
export default {
  ...
  methods: {
    ...
    submitForm (PicArticleForm) {
      this.$refs[PicArticleForm].validate(valid => {
        if (valid) {
          // eslint-disable-next-line no-unused-vars
          let res
          if (this.id) {
            // 如果有id,则编辑
            res = this.$http.put(`rest/picarticles/${this.id}`, this.model)
          } else {
            res = this.$http.post('rest/picarticles', this.model)
          }
